...
$(x = a_{(1,1)}, y = b_{(1,1)}, z = b_{(0,2)})$. Eines der augenf"alligsten 
Ergebnisse ist, das ab $k = 4$ das chaotische Verhalten zu verschwinden 
scheint. Was daran erkennbar ist, dass auf der linken Seite keine abrupten 
Wechsel mit gleicher Amplitude zu sehen sind, sondern alle L"osungen entweder 
konvergieren oder in eine stabile Schwingung "uber gehen. Auf der rechten Seite 
verschwinden dadurch die f"ur ein Lorenzsystem typischen 
``Schmetterlingsfl"ugel''.
...
Anfangsbedingungs- und Parameterraum nicht untersucht haben und es zwar 
chaotische L"osungen g"abe, unsere Konstellation aber keine davon ist.
